...
def download_csv(dir, usaf, url):
    for year in range(1994, 2025):
        ...
        if response.status_code == 200:
            ...
            for link in links:
                if link['href'].startswith(str(usaf)) and link['href'].endswith(".csv"):
                    csv_file_url = csv_url + link['href']
                    ...
                    if csv_response.status_code == 200:
                        ...
                        print(f"Failed to download CSV for {usaf} in {year}")
        else:
            print(f"Failed to access {csv_url}")
...
# URL where the CSV files are hosted
